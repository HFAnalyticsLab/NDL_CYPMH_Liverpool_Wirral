...
#raw dummy data
d <- fread("./topic2_github_material/dummy_CYPMH_data.csv")
names(d)[47] <- 'New_LSOA'
...
sp_lsoa <- st_read(normalizePath("./Data/shp/Wirral_Liverpool_LSOA_bnds.shp"))
sp_lsoa <- sp_lsoa[sp_lsoa$LSOA11CD %in% lsoa_age2_sex$lsoa11, ]
liv_wirral<-lk_lsoa_msoa_la[ladnm=="Liverpool"|ladnm=="Wirral"]

#get local data
lk_lsoa_msoa_la<- fread("./Data/lk_lsoa_msoa_la.csv")

#limit lsoa_datato liverpool wirral 
lsoa_data<-lsoa_data[lsoa11 %in% liv_wirral$lsoa11]


#get single year population data
#lsoa_sy_pop is a long-form data table with national LSOAs, age bands 0-90, count for each sex in 2019 
lsoa_sy_pop<-fread("./Data/mipop2019_sy.csv")
#extract Liverpool & Wirral LSOAs
lsoa_sy_pop<-lsoa_sy_pop[`LSOA Code` %in% liv_wirral$lsoa11]
...
# derive year and month - dont convert to strings and sub string its very slow - use data.table commands
...
# A&E data only good from 2019 onward - probably best to juts use data from begining of 2019
#NOT NEEDED d<-d[year>2018]
...
#collapse to counts per single age
table(d$ecds_eating_disorder, useNA = "ifany")
...
#age specific trends = 2 age groups

lsoa_pop <- fread("./Data/pop2017_2020__mf.csv")
lsoa_pop <- lsoa_pop[lsoa11 %in% liv_wirral$lsoa11]
#lsoa_pop <- lsoa_pop[year==2019] #NOTE cuts out 2020 needed for covid analysis
...
#replicate 2021 data as 2020
#pop2021<-pop2[year==2020]
#pop2021[, year:=2021]
#append
#ls<-list(pop2,pop2021)
#pop2<-rbindlist(ls)
...
table(age_sex_month$age_group, useNA = "ifany")
#calculate rates
...
# descriptive analysis -  2 age groups by LSOA across 2019
...
lsoa_pop[,  age_group2:=cut(age_cat, breaks=c(0,15,25), include.lowest=T, right=F)]
#to account for person year - and half year in 2021
...
# output data for HF
#names(imd_ae[,1:18])
#out_imd_ae<-imd_ae[value<5, value:=NA]
#imd_ae[, rate:=NULL]

#Figure table: Figure 5 (<5 removed)
fwrite(imd_ae, "./national_imd_Figure_5_ae_by_reason_and_imd_DUMMY.csv")
...
# output data for HF
#names(imd_adm[,1:18])
#out_imd_adm<-imd_adm[value<5, value:=NA]
#imd_adm[, rate:=NULL]

#Figure table: Figure 6 (<5 removed)
...
#Figure 9
#bivariate map showing areas with higher or lower rates
map2 <- ggplot() +
...
#regression models - cross sectional - what rates are associated with health indices (CCG)?

centre<-function(x) {(x-mean(x,na.rm=T))/sd(x, na.rm=T)}


cols<-c("CNI_Score","prop_ibesa",
        "imd","samhi_index","prop_white","prop_u16","prop_o75","gpperpop",
        "ed_dist","gpp_dist.x","green_pas",
        "pm10_mean","prop_1less", "lone_parent")
lsoa_age2_sex[, (cols):=lapply(.SD, centre), .SDcols=cols]

lsoa_age2_sex[, rate:=fc_flag/an_pop]
table(lsoa_age2_sex$age_group2)
m0<-glm(fc_flag~1+offset(log(an_pop)),data=lsoa_age2_sex[age_group2=="[0,15)"], family = poisson(link=log))
m1<-glm(fc_flag~sex+CNI_Score+
...
table(lsoa_age2_sex$sex)
summary(m1)
1-logLik(m1)/logLik(m0)

m0<-glm(fc_flag~1+offset(log(an_pop)),data=lsoa_age2_sex[age_group2=="[15,25]"], family = poisson(link=log))

m2<-glm(fc_flag~sex+CNI_Score+
          imd+prop_white+gpperpop+
...
#ggsave("./Outputs/Figures/Figure10_Reg_res_all_contact_DUMMY.png", width = 9, height = 6, units = 'in')
...
#fwrite(results[, .(term,`estimate_Age 0-14`,`conf.low_Age 0-14`,`conf.high_Age 0-14`,
 #                  `estimate_Age 15-25`,`conf.low_Age 15-25`,`conf.high_Age 15-25`)], "./Outputs/tables/national_imd_reg_first_contact_Figure10_DUMMY.csv")


#  regression area based association with a&e emergency admissions 

m0<-glm(ad_em+ae~1+offset(log(an_pop)),data=lsoa_age2_sex[age_group2=="[0,15)"], family = poisson(link=log))
...
m0<-glm(ad_em+ae~1+offset(log(an_pop)),data=lsoa_age2_sex[age_group2=="[15,25]"], family = poisson(link=log))
...
#  regression area based association with referrals

#m0<-glm(referrals~1+offset(log(an_pop)),data=lsoa_age2_sex[age_group2=="[0,15)"], family = poisson(link=log))
#m1<-glm(referrals~sex+CNI_Score+
 #         imd+prop_white+gpperpop+
  #        green_pas+lone_parent+offset(log(an_pop)),data=lsoa_age2_sex[age_group2=="[0,15)"], family = poisson(link=log))
...
#m0<-glm(referrals~1+offset(log(an_pop)),data=lsoa_age2_sex[age_group2=="[15,25]"], family = poisson(link=log))

#m2<-glm(referrals~sex+CNI_Score+
 #         imd+prop_white+gpperpop+
  #        green_pas+lone_parent+offset(log(an_pop)),data=lsoa_age2_sex[age_group2=="[15,25]"], family = poisson(link=log))
#summary(m2)
#1-logLik(m2)/logLik(m0)
...
#CHECK - is this included in the report?
#ggplot(results, aes(y=result, x=estimate, group=1)) +
#  geom_point(shape=21, size=3, fill="white") +
 # geom_vline(xintercept = 1, color="red", linetype="dashed")+
  #geom_errorbarh(height=.1, aes(xmin=conf.low, xmax=conf.high)) +facet_wrap(out~.)+theme_minimal(base_size = 15)+
  #ylab("")+xlab("relative effect for each sd")+ggtitle("Referrals to mental health services")

#ggsave("./Outputs/Figures/reg_res_contacts.png", width = 9, height = 6, units = 'in')

#results<-dcast.data.table(results,term~out, value.var = c("estimate", "p.value", "conf.low", "conf.high"))

#Figure table
#fwrite(results[, .(term,`estimate_Age 0-14`,`conf.low_Age 0-14`,`conf.high_Age 0-14`,
 #                  `estimate_Age 15-25`,`conf.low_Age 15-25`,`conf.high_Age 15-25`)], "./Outputs/tables_for_hf/reg_referrals.csv")
#
...
#pop2021<-pop_lsoa_age_month[year==2020]
#pop2021[, year:=2021]
#append
#ls<-list(pop_lsoa_age_month,pop2021)
#pop_lsoa_age_month<-rbindlist(ls)
#table(pop_lsoa_age_month$age_group)
...
#LOSES ALL YEARS expect 2019 HERE...???
#pop_lsoa_age_month only 2019...
...
trend_ae<-trend_d[, list(ad_em=sum(ad_em), an_pop=sum(an_pop)), by=.(covid,time,yearmon, natn_terc, age_group2,sex)]

trend_ae[, exp_ad:=predict(model1, type="response", newdata = trend_ae, se.fit =T)[1]]
trend_ae[, se:=predict(model1, type="response", newdata = trend_ae, se.fit =T)[2]]
trend_ae[, lcl:=exp(log(exp_ad)-2*(log(se)))]
trend_ae[, ucl:=exp(log(exp_ad)+2*(log(se)))]

#Figure table: Figs 12 & 13
fwrite(trend_ae, "./Figures_12_13_trend_aeate_age_imd_DUMMY.csv")
...
#Figure 12
ggplot(data=trend_ae, aes(y=exp_ad, x=yearmon, color=as.factor(natn_terc), group=natn_terc))+
...
trend_ct<-trend_d[, list(ad_em=sum(ad_em), an_pop=sum(an_pop)), by=.(covid,time,yearmon, natn_terc, age_group2,sex)]
